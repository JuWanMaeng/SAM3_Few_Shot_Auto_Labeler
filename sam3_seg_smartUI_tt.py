...
# [추가됨] 참조(Reference) 마스크 저장 경로
REF_MASK_OUTPUT_DIR = os.path.join(BASE_OUTPUT_DIR, "ref_masks")
REF_OVERLAY_OUTPUT_DIR = os.path.join(BASE_OUTPUT_DIR, "ref_overlays")
...
for path in [TEMP_DIR, MASK_OUTPUT_DIR, OVERLAY_OUTPUT_DIR, BBOX_OUTPUT_DIR, 
             ID_MASK_OUTPUT_DIR, REVERSE_MASK_OUTPUT_DIR, REVERSE_BBOX_OUTPUT_DIR, 
             REVERSE_OVERLAY_OUTPUT_DIR, REF_MASK_OUTPUT_DIR, REF_OVERLAY_OUTPUT_DIR]:
...
class InteractiveBatchLabeler:
    def __init__(self):
        ...
        self.ref_frames = []
        self.ref_names = [] # [추가됨] 참조 이미지 파일명 저장
        self.ref_prompts = [] 
        self.id_thumbnails = {}
        self.id_counts = {}
    ...
    def prepare_references(self):
        ...
        for i, path in enumerate(REF_IMAGES):
            ...
            self.ref_names.append(os.path.basename(path)) # [추가됨] 파일명 저장
    ...
    def run_fast_inference(self, target_files):
        ...
        for _ in self.predictor.propagate_in_video(inference_state, start_frame_idx=0, max_frame_num_to_track=dummy_idx, propagate_preflight=True, reverse=True): pass
        ...
        with torch.inference_mode():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                # [추가됨] =========================================================
                # Reference(메모리) 이미지의 마스크를 추출하여 저장하는 로직
                # ================================================================
                    ...
                    for oid in tracked_ids:
                        ...
                        if pred_masks is not None:
                            ...
                            if mask_bool.ndim > 0 and mask_bool.any():
                                detected_ref_objs.append({'id': oid, 'mask': mask_bool, 'score': 1.0})
                                if combined_ref_mask is None: combined_ref_mask = np.zeros_like(mask_bool, dtype=bool)
                                combined_ref_mask = np.maximum(combined_ref_mask, mask_bool)

                    # 저장
                    if combined_ref_mask is not None:
                        if combined_ref_mask.ndim == 3: combined_ref_mask = combined_ref_mask.squeeze()
                        final_ref_mask = cv2.resize(combined_ref_mask.astype(np.uint8), ref_orig_size, interpolation=cv2.INTER_NEAREST)
                        
                        # Mask 저장
                        Image.fromarray((final_ref_mask * 255).astype(np.uint8)).save(os.path.join(REF_MASK_OUTPUT_DIR, base_r_name + "_ref_mask.png"))
                        
                        # Overlay 저장
                        ref_pil = Image.fromarray(self.ref_frames[r_idx])
                        if detected_ref_objs:
                            vis_ref = self.create_overlay_with_id(ref_pil, detected_ref_objs, ref_orig_size)
                            vis_ref.save(os.path.join(REF_OVERLAY_OUTPUT_DIR, base_r_name + "_ref_overlay.jpg"))
                ...
                for t_idx, t_path in enumerate(target_files):
                    ...
                    try:
                        ...
                        for oid in tracked_ids:
                            ...
                            pred_masks = current_out["pred_masks"]
                            obj_score = current_out["object_score_logits"] # shape check needed
                            iou_score = current_out["iou_score"]       # shape check needed

                            # [수정됨] 안전장치: 1차원(Vector)으로 나오면 2차원(Batch)으로 변환
                            ...
                            if pred_masks is not None:
                                # 여기서 iou_score는 이제 무조건 2차원 [1, 3]이므로 dim=1 에러가 안 납니다.
                                if pred_masks.shape[1] > 1:
